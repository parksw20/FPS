  ...
  if (showFps) {                        // 최근 0.5초 평균
    ...
    if (fpsAcc >= 0.5) {
      ...
      document.getElementById('fpsHud').textContent = Math.round(fpsN / fpsAcc) + ' FPS · ' + (fpsAcc / fpsN * 1000).toFixed(1) + 'ms'
        ...
        + ' · 화면 ' + innerWidth + '×' + innerHeight + ' @' + (devicePixelRatio || 1).toFixed(2);   // UI 크기 진단용
      fpsAcc = 0; fpsN = 0;
    }
  }
...
const FURN = {
  ...
  banner: {
    name: '배너', icon: '<svg viewBox="0 0 24 24" width="52" height="52" aria-hidden="true"><rect x="2.5" y="4" width="19" height="16" rx="2" fill="#e8edf4" stroke="#8a929c" stroke-width="1.4"/><rect x="4.5" y="6" width="15" height="12" fill="#6fc3ff"/><path d="M4.5 18 L10 11.5 L13.5 15 L15.5 13 L19.5 18 Z" fill="#3a9a5c"/><circle cx="15.5" cy="9" r="1.8" fill="#ffd76b"/></svg>', w: 0.9, d: 0.06, h: 1.4, color: 0x2b6f8f,
